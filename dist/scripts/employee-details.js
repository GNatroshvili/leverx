document.addEventListener("DOMContentLoaded", () => {
    ...
    const detailsEmployeeAvatar = document.getElementById("details-employee-avatar");
    const detailsEmployeeName = document.getElementById("details-employee-name");
    const detailsEmployeeNativeName = document.getElementById("details-employee-native-name");
    const detailsRemoteWorkIcon = document.getElementById("details-remote-work-icon");
    ...
    function populateEmployeeDetails(employee) {
        ...
        if (detailsEmployeeAvatar)
            detailsEmployeeAvatar.src = employee.user_avatar;
        if (detailsEmployeeName)
        ...
        if (detailsRoom)
            detailsRoom.textContent = employee.room || "N/A";
        ...
        // update contact info
        if (detailsPhone)
            detailsPhone.textContent = employee.phone || "N/A";
        if (detailsEmail)
            detailsEmail.textContent = employee.email || "N/A";
        if (detailsSkype)
            detailsSkype.textContent = employee.skype || "N/A";
        if (detailsCNumber)
            detailsCNumber.textContent = employee.cnumber || "N/A";
        // update travel info
        ...
        if (employee.visa && employee.visa.length > 0) {
            ...
            if (detailsVisa1Period)
                detailsVisa1Period.textContent = `${formatDate(visa1.start_date)} - ${formatDate(visa1.end_date)}${visa1Expired ? " (expired)" : ""}`;
            if (employee.visa.length > 1) {
                ...
                if (detailsVisa2Period)
                    detailsVisa2Period.textContent = `${formatDate(visa2.start_date)} - ${formatDate(visa2.end_date)}${visa2Expired ? " (expired)" : ""}`;
            }
        }
        else {
            // no visa data available
            if (detailsVisa1)
                detailsVisa1.textContent = "N/A";
            if (detailsVisa1Period)
                detailsVisa1Period.textContent = "";
            if (detailsVisa2)
                detailsVisa2.textContent = "N/A";
            if (detailsVisa2Period)
                detailsVisa2Period.textContent = "";
        }
    }
